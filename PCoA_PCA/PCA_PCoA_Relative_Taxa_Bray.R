...
# Script name: Rcode_PCoA_Relative_Taxa_Bray
# Created on: 24 February 2024
# Last updated: 26 April 2024
# Author: Neeraja Balasubrahmaniam and Jon C. King
# Description: This script performs PCA on gene expression and PCoA on relative
# abundance of taxa (ASVs, species, genus) data using Bray-Curtis distances.
...
data_Spec <- data %>%
  group_by(Species) %>%
  summarise(across(where(is.numeric), sum))
d_list <- list(data_ASV, data_Spec, data_Gen)
d_list2 <- list()
d_list3 <- list()
for (i in seq(1:3)) {
  d_list2[[i]] <- d_list[[i]] %>%
...
d_list3 <- lapply(d_list2, function(x) vegdist(x[,-c(1,2)], method = "bray")) 
...
letters1 = c("C", "A", "B") # initialize for plot labels
...
for (i in seq(1:3)) {
  ...
  d_list4[[i]] <- #ggplot(data = df) +
    ggplot(df, aes(x = Axis.1, 
    ...
    stat_ellipse(geom = "polygon",
                 aes(fill = ERH), 
                 alpha = 0, linewidth = 1) +
    #geom_point(mapping = aes(x = Axis.1, y = Axis.2, color = RH), size = 3) +
    #labs(title = names(d_list3)[i], 
...
data = read.table("RSEM.gene.counts.matrix",           # load gene counts table
...
#---- Get a samples-to-RH file in the same order as colnames of prin_comp_data
...
letters1 = c("E", "C", "D") # change based on plot order preference

my_cols = c("#FFD92F", "#FC8D62", "#8DA0CB","#E78AC3","#A6D854","#66C2A5"
            ,"#00bb00", "#B3B3B3",  "#E5C494")  
...
# To plot figure S2
p6 = ggdraw() +
  draw_plot(d_list4[[2]], x = 0, y = 0.5, width = .5, height = 0.5) +
  
  draw_plot(d_list4[[3]], x = 0.5, y = 0.5, width = .5, height = 0.5) +

  draw_plot(d_list6[[2]], x = 0, y = 0, width = .5, height = 0.5) +
  
  draw_plot(d_list6[[3]], x = .5, y = 0, width = .5, height = 0.5) 

p6

# To plot figure 3: 
# use limits mentioned for figure 2 previously before plotting this
# modify plot labels (A, B, C, D) using Inkscape 

p7 = ggdraw() +
  draw_plot(d_list5[[1]], x = 0, y = 0.5, width = .5, height = 0.5) +
  
  draw_plot(d_list4[[1]], x = 0.5, y = 0.5, width = .5, height = 0.5) +
  
  draw_plot(d_list7[[1]], x = 0, y = 0, width = .5, height = 0.5) +
  
  draw_plot(d_list6[[1]], x = .5, y = 0, width = .5, height = 0.5) 
